...
public class BoltPrimer extends BaseBasicBolt {
    ...
    public void prepare(Map stormConf, TopologyContext context) {
        ...
        fileName = String.valueOf(Objects.requireNonNull(stormConf.get("demo.file.out")));
    }

    @Override
    public void execute(Tuple input, BasicOutputCollector collector) {
        try(final PrintWriter printWriter = new PrintWriter(new FileWriter(fileName, true))) {
            final String line = "processing line " + new Date().getTime() + ": "+ input.getStringByField("line");
            printWriter.println(line);
            log.info("line written {} to {}", line, fileName);
        } catch (Exception e) {
